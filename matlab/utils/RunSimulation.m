...
tout = zeros(length(trajectory.time), 1);
yout.x = zeros(length(trajectory.time), length(quad_current_state));
yout.u = zeros(length(trajectory.time), 4);

% Measure total simulation time
total_sim_time = 0.0;

control_period = trajectory.time(2) - trajectory.time(1);
for current_idx = 1:size(trajectory.states, 1)
